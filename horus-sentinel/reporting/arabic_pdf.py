...
class _Report(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(
            self.w - 32,
            10,
            ar("حورس سنتينل — تقرير استخبارات مفتوحة المصدر"),
            align="R",
            new_x="LMARGIN",
            new_y="NEXT",
        )
    ...
    def section_title(self, num: int, title: str) -> None:
        ...
        self.cell(0, 9, ar(f"{title}  .{num}"), align="R", fill=True, new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*_INK)
        self.ln(1)
    ...
    def finding(self, f: dict[str, Any]) -> None:
        ...
        self.multi_cell(
            ...
            6.5,
            ar(
                f"{f.get('title', '')}  —  خطورة {_band_label(band)} ({f.get('risk_score', 0):.1f})"
            ),
            align="R",
            new_x="LMARGIN",
            new_y="NEXT",
        ...
        if fw:
            ...
            self.multi_cell(
                ...
                6,
                ar(
                    f"الإطار المرجعي: {fw.get('technique_id', '')} "
        ...
        self.multi_cell(
            0,
            6,
            ar(f"التوصية: {f.get('recommendation', '')}"),
            align="R",
            new_x="LMARGIN",
            new_y="NEXT",
        )
...
def render_arabic_pdf(context: dict) -> bytes:
    ...
    pdf.cell(
        0,
        9,
        ar(f"الموضوع: {subject_value}  ({_TYPE_AR.get(subject_type, subject_type)})"),
        align="R",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    pdf.cell(
        ...
        6,
        ar(
            f"رقم المهمة: {context.get('job_id', '')} · التوليد: "
    ...
    pdf.muted(
        f"{context.get('entity_count', 0)} كيان · " + "، ".join(f"{v} {k}" for k, v in hist.items())
    )
    if entities:
        rows = [
            [
    ...
    if hits:
        ...
        pdf.table(["IP", "الخدمة", "التقنية", "CVE", "CVSS"], rows, [0.24, 0.18, 0.24, 0.22, 0.12])
    else:
    ...
    pdf.muted(
        f"{len(graph.get('nodes', []))} عقدة · {len(graph.get('edges', []))} علاقة · "
    ...
    if risk:
        rows = [
            [
                ...
                _band_label(r.get("risk_band", "Info")),
            ]
            for r in risk
        ]
    ...
    for adj in card.get("band_adjustments", []):
        pdf.muted(
            f"تعديل النموذج (±1، مُسجَّل): {adj.get('entity_key', '')}: "
    ...
    if audit:
        rows = [
            [
                ...
                str(a.get("subject", "")),
                "إصابة" if a.get("cache_hit") else "جلب",
                str(a.get("recorded_at", ""))[:19],
            ]
            for a in audit
        ]
        pdf.table(
            ["الأداة", "المصدر", "الموضوع", "الذاكرة", "الوقت"], rows, [0.18, 0.2, 0.24, 0.13, 0.25]
        )
    pdf.muted(
        f"التفويض: موقّع من {roe.get('signed_by', '')}، ينتهي {str(roe.get('expires_at', ''))[:19]}."
    )
